function [Iload] = Ibusload(System,vPr)
Sbase=System.BaseVA;
NumN = System.NumN;
Ibusstar = zeros(NumN*3,1);
Ibusdelta = zeros(NumN*3,1);
for k = 1:System.NumC
      N1 = System.Loads(k,1); %node of load          
      star = System.Loads(k,2);%star  if value=1, Delta if value =0
      alpha = System.Loads(k,3); %type... PQ=0,I=1,Z=2   
      Sstar = zeros(3,1);
      Sdelta    = zeros(3,1);
      if star == 1
         Sstar(1) = (System.Loads(k,4) + 1j*(System.Loads(k,5)))/Sbase; 
         Sstar(2) = (System.Loads(k,6) + 1j*(System.Loads(k,7)))/Sbase; 
         Sstar(3) = (System.Loads(k,8) + 1j*(System.Loads(k,9)))/Sbase; 
      else
         Sdelta(1) = (System.Loads(k,4) + 1j*(System.Loads(k,5)))/Sbase; 
         Sdelta(2) = (System.Loads(k,6) + 1j*(System.Loads(k,7)))/Sbase; 
         Sdelta(3) = (System.Loads(k,8) + 1j*(System.Loads(k,9)))/Sbase;                
      end
       V=vPr(3*N1-2:3*N1);   
